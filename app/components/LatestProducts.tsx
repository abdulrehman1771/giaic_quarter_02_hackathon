    ...
    return (
      ...
      <div className="py-12">
        ...
        <div className="flex justify-center space-x-8 text-sm mb-8">
          <button className="text-pink-500 font-semibold underline">New Arrival</button>
          <button className="text-gray-700 hover:text-indigo-900">Best Seller</button>
          <button className="text-gray-700 hover:text-indigo-900">Featured</button>
          <button className="text-gray-700 hover:text-indigo-900">Special Offer</button>
        </div>
        <div className="grid grid-cols-3 gap-8 max-w-6xl mx-auto">
          {products.map((product) => (
            <div key={product.id} className="relative bg-white rounded-lg overflow-hidden">
              <img src={product.image} alt={product.name} className="bg-[#F7F7F7] w-full h-64 object-cover" />
              {product.isNew}
              {product.isOnSale && (
              ...
              <div className="flex gap-5 p-4 items-center justify-between text-center">
                <h3 className="font-medium border-b-2 text-lg text-gray-800">{product.name}</h3>
